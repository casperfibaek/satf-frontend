...
function searchHeaderForCoordinates(header:string[]) {
  // Common phrases.
...
function removeInvalidRowsAndCols(arr:any[]):any[] {
...
function renameDuplicate(name:string, array:string[], prefix:string = '_'):string {
...
function uniqueHeader(header:string[]):string[] {
...
function generatePoint(coords:number[], properties:any = {}):GeoJsonFeature {
...
function generateGeojson(geometryArray:number[][], propertiesArray:any[]): GeoJsonFeatureCollection {
...
async function arrayToGeojson(excelCells:any[][]) {
  ...
  const selected: { name: any, col: any, amount: any } = {
    name: null,
    col: null,
    amount: null,
  };
  ...
  if (selected.name === 'what3words' || selected.name === 'pluscodes') {
    ...
    geometry = await Promise.all(geometry.map((g:any) => g.json()));
  }

  if (validatedRows.length === 0) { return false; }
